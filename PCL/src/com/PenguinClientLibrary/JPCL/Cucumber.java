...
public class Cucumber implements Runnable {
	...
	public String sendAndWait(String data, String needle){
		...
		//If for some reason its still null wait another second
		if (response == null) {
			sleep(1);
			response = searchRecvBuffer(needle);
		}
	...
	public boolean connect(String username, String password, int server) {
		String loginKey, randKey = null, data = null;
		...
		try {		
			...
			sendAndWait("<msg t='sys'><body action='verChk' r='0'><ver v='153' /></body></msg>", "apiOK");
			
			//rndk
			notification("Sending rndK check...");
			data = sendAndWait("<msg t='sys'><body action='rndK' r='-1'></body></msg>", "rndK");
			...
			data = sendAndWait("<msg t='sys'><body action='login' r='0'><login z='w1'><nick><![CDATA[" + username + "]]></nick><pword><![CDATA[" + passwordHash + "]]></pword></login></body></msg>", "%l%-1%");
			...
			sendAndWait("<msg t='sys'><body action='login' r='0'><login z='w1'><nick><![CDATA[" + username + "]]></nick><pword><![CDATA[" + passwordLoginHash + "]]></pword></login></body></msg>", "%l%-1%");


			int currentPacket = (packetNumber + 769567) ^ 942215; //205881
			packetNumber += 1;
			
			sendAndWait("%xt%s%j#js%" + currentPacket + "%-1%"+ myPlayerID +"%" + loginKey + "%en%", "js");
	...
	public Player getPlayerByName(String name) throws InterruptedException {
		...
		response = sendAndWait("%xt%s%u#pbn%-1%" + name + "%", name);
	...
	public void handlePacket(String packet) {
		...
		if (packetArray[1].equals("xt")) {
			...
			// Join Game handler
				...
				this.purgePlayers();
				this.players.get(myPlayerID).setRoom(Integer.parseInt(packetArray[3]), Integer.parseInt(packetArray[4]));
